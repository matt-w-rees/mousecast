predict_summarise_posterior_draws_by_ae_zone <- function(model_draws, model_data, aez_adj,
                                                         low_cutoff = 0.25, high_cutoff = 0.75,
                                                         spatial = TRUE
){
  
  
  # RENAME LISTS FROM TRENDS TO ae_zoneS -------------------------------------
  
  # make a look-up table for trends and ae_zone pairs
  map_ae_zone_to_trend_in_order <- as.data.frame(names(model_draws)) |> 
    rename("trend" = "names(model_draws)") |>
    left_join(unique(dplyr::select(model_data, trend, ae_zone)), by = "trend")
  
  # rename lists to ae_zone using look-up table
  names(model_draws) <- map_ae_zone_to_trend_in_order$ae_zone
  
  
  # COLLAPSE LISTS FROM TREND TO ae_zone -------------------------------------
  # For each ae_zone, convert each site's draws to the response scale and then
  ...
  # A cheap first pass (colMeans per group) gives pred_mean for all ae_zones
  # and times so the global quantile can be derived before extract_preds runs.
  ...
  preds_df <- data.table::rbindlist(preds_list, idcol = TRUE) |>
    # the name of each draw is added as a column - this is our ae_zone list name so rename accordingly
    rename(ae_zone = .id) |>
  ...
  #--------------------------------------------------------------
  # by comparing to lagged value for each ae_zone (already ordered chronologically)
  preds_df <- preds_df |>
  ...
  if (spatial == TRUE) {
    ...
    # add ae_zone polygon geometries to preds
    preds_df <- st_as_sf(left_join(preds_df, aez_adj, by = "ae_zone"))
