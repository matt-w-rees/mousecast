# Combine and summarise rapid assessment data from all sources.
# Takes the three source targets separately so each can be tagged with its
# target name as data_source before binding.
# Computes burrow and chewcard summary statistics and renames columns to the
# format expected by downstream pipeline functions.

data_rapid_summarise <- function(access_rapid, csv_rapid, odk_rapid) {

  # Standardise free-text crop_type/crop_stage to ODK controlled vocabulary before
  # binding; odk_rapid already uses the ODK schema (crop_group, crop_variety, crop_stage)
  data <- dplyr::bind_rows(
    access_rapid |> standardise_crop_variables() |> dplyr::mutate(data_source = "data_access_monitoring_rapid"),
    csv_rapid    |> standardise_crop_variables() |> dplyr::mutate(data_source = "raw_data_csv_rapid"),
    odk_rapid    |> dplyr::mutate(data_source = "raw_data_odk_rapid")
  ) |>
    # Drop unnamed columns (e.g. row-index column in raw_data_csv_rapid)
    dplyr::select(-dplyr::any_of(c("", NA_character_)))
  ...
  data |>
    transmute(
      ...
      # crop information (standardised to ODK controlled vocabulary)
      crop_group,
      crop_variety,
      crop_stage,
      biomass,
