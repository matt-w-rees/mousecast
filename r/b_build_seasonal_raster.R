...
#
# Distinct from build_seasonal_window_raster() (r/b_build_seasonal_window_raster.R),
# which collapses to one *fixed calendar window* (e.g. rainfall's Apr-Oct
# growing-season total) per plain calendar year -- this instead produces a
# value for *every* one of the four seasons, keyed by season_year_adj, for
# covariates that need a real value at every seasonal time step rather than
# one fixed-window value carried forward across the year.
#
...
# "Winter-2026"), matching build_seasonal_gpp_raster()'s own season-label
# convention (r/b_build_seasonal_gpp_raster.R) so this can be attached the
...
# summarise_silo_to_month()'s own min_days_per_month, build_seasonal_gpp_raster()'s
...
build_seasonal_raster <- function(rast, summary_func = c("mean", "sum")) {
  ...
  year_adj <- ifelse(months == 12L, years + 1L, years)

  season_year_adj <- paste0(season, "-", year_adj)
  complete_labels <- names(which(table(season_year_adj) == 3))

  seasonal_rast <- purrr::map(complete_labels, function(lbl) {
    idx <- which(season_year_adj == lbl)
