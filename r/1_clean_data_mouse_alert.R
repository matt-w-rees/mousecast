...
clean_data_mouse_alert <- function(path) {
  ...
  dup_groups <- cleaned |>
    dplyr::group_by(longitude, latitude, survey_date, observer_name) |>
  ...
  if (n_discarded > 0) {
    message(sprintf(
      "clean_data_mouse_alert(): merging %d duplicate report(s) (same location, date & observer) into %d record(s)",
      n_discarded, dplyr::n_groups(dup_groups)
    ))
  }

  # --- 8. Merge duplicate reports into a single record -----------------------
  # Collapse to one row per (longitude, latitude, survey_date, observer_name),
  # keeping the highest reported abundance and combining observation IDs /
  # comments from the merged records.
  cleaned <- cleaned |>
    dplyr::group_by(longitude, latitude, survey_date, observer_name) |>
    dplyr::summarise(
    ...
    dplyr::relocate(
      observation_id, data_source, project, longitude, latitude, state,
